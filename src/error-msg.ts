...
export const fileErrorMsgs: { [errorCode in FileCheckResponse]: string } = {
  ...
  [FileCheckResponse.FILE_OUTSIDE_BASE]: "Link references a file that exists outside of the base directory.",
...
export const anchorErrorMsgs: { [errorCode in AnchorCheckResponse]: string } = {
  ...
  [AnchorCheckResponse.BINARY_FILE]: "Link targets a binary file, which means there is no useful way to target individual sections of the file with an anchor.",
  [AnchorCheckResponse.ANCHOR_UNDISCOVERABLE]: "Link targets a file with no file extension, so it cannot be determined if the anchor is valid.",
  [AnchorCheckResponse.NO_ANCHORS_IN_FILETYPE]: "Link targets a non-document file with an anchor that isn't supported for non-document files.",
  ...
  [AnchorCheckResponse.MULTI_LINE_TARGET_RANGE_INVALID]: "The start number of a multi-line anchor must be less than the end number.",
};
